...
const projects = [
  {
    ...
    progress: 100,
    techStack: ["React", "TailwindCSS", "Node.js", "Express", "Sqlite"],
...
function Projects() {
  return (
    ...
    <div className="w-50/100 ">
      <button className="my-4 rounded-full bg-[#3939395C] px-4 py-2 text-[#FF9853] new-font-ibm">Projects I Had Worked On</button>
      <h4 className="text-4xl font-semibold wrap-break-word text-[#FFFFFF] md:w-70/100 new-font">{project.title}</h4>
      <div id="project" className="my-8 flex h-auto w-70/100 flex-col">
        <div className='flex space-x-4 w-full items-center'>
          <button className="max-w-[120px] rounded-full bg-[#FF9853] px-4 py-2 text-white  new-font-ibm"><a href={`${project.projectLink}`}>Project Link</a></button>
          <button className='max-w-[120px] rounded-full border-2 border-[#FF9853] px-4 py-2 text-[#FF9853]  new-font-ibm'><a href={`${project.caseStudyLink}`}>Case Study</a></button>
        </div>
        <p className="mt-4 min-w-full wrap-anywhere text-white">Project {project.id} - {project.description}</p>
        <div className="flex w-full flex-col">
          <div className="mt-4 flex w-full justify-between">
            <h6 className="text-gray-700 new-font-ibm">Progress</h6>
            <h6 className="text-white new-font-ibm">{project.progress}%</h6>
          </div>
        ...
        <div className="mt-4 w-full">
         ...
         { project.techStack.map((tech,index)=>(
            ...
            <p className="font-semibold text-gray-600 uppercase new-font-ibm">{tech}</p>
          </div>
          )) }
        </div>
      </div>
